...
def get_attack(attack, fmodel):
    ...
    if attack == 'PA':
        ...
        A = fa.EADAttack(decision_rule="L1")
...
def test_foolbox(model_name, max_tests, model_test):
    ...
    epsilons_dict = {0:10, 1:10, 2:2.0, 3:0.3}
    attacks_dist = []
    ...
    for i in range(len(attacks_list)):
        ...
        for images,labels in test_loader:        
            ...
            deltas = advs_final - images #Now remove the attacks that did not belong to the allowable perturbation region
            multiplier = (norm(deltas).clamp(max=epsilons_dict[types])).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            deltas_final = multiplier * deltas/norm(deltas).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            advs_final = images + deltas_final
            classes = model_test(advs_final)
            num_correct = classes.max(1)[1] == labels
            output[total:total+batch_size] = num_correct.cpu()
            total += batch_size
...
parser.add_argument("-subset", help = "restarts", type = int, default = 1)
